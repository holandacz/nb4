...
*
* @package	vBulletin
* @version	$Revision: 27960 $
* @date		$Date: 2008-09-29 09:28:11 -0500 (Mon, 29 Sep 2008) $
*/
	...
	*/
	function fetch_best_resize(&$jpegconvert, $resize = true)
	{
		...
		else if ($jpegconvert AND $this->upload['resized']['filesize'] AND ($this->upload['resized']['filesize'] > $this->maxuploadsize OR $forceresize))
		{
			...
			$this->upload['resized'] = $this->image->fetch_thumbnail($this->upload['filename'], $this->upload['location'], $newwidth, $newheight, $this->registry->options['thumbquality']);
			if (empty($this->upload['resized']['filedata']))
			{
...
|| # CVS: $RCSfile: class_upload.php,v $ - $Revision: 27960 $
|| ####################################################################
\*======================================================================*/
?>