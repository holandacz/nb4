...
$query = $vbulletin->db->query_read("SELECT total_votes, total_value, userids FROM " . TABLE_PREFIX . "goldbrick_rating WHERE id='$id_sent' ")or die(" Error: ".mysql_error());
...
if(!$voted) {     //if the user hasn't yet voted, then vote normally...
	...
	if (($vote_sent >= 1 && $vote_sent <= $units)) { // keep votes within range, make sure IP matches - no monkey business!
		$update = "UPDATE " . TABLE_PREFIX . "goldbrick_rating SET total_votes='".$added."', total_value='".$sum."', userids'".$userid."' WHERE id='$id_sent'";
		$result = $vbulletin->db->query_read($update);		
	} 
} //end for the "if(!$voted)"
// these are new queries to get the new values!
$newtotals = $vbulletin->db->query_read("SELECT total_votes, total_value, userids FROM " . TABLE_PREFIX . "goldbrick_rating WHERE id='$id_sent' ")or die(" Error: ".mysql_error());
$numbers=$vbulletin->db->fetch_array($newtotals);
$count = $numbers['total_votes'];//how many votes total
$current_rating = $numbers['total_value'];//total number of rating added together and stored
$tense = ($count==1) ? "vote" : "votes"; //plural form votes/vote
