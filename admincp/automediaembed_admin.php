<?php
	...
	//  AME Autmoatic Media Embedder 2.5.0
	...
	if ($action == 'dorebuild')
	{
		...
		switch ($zone)
		{
			...
			case 'sig':
				...
				$sql = "SELECT count(userid) total FROM " . TABLE_PREFIX . "usertextfield WHERE 1=1  AND (signature LIKE '%[/url]%' OR signature LIKE '%[/ame]%' OR pagetext LIKE '%[/nomedia]%') $and ";
				break;
			...
			switch ($zone)
			{
				...
				case 'sig':
					...
					$sql = "SELECT
							...
							WHERE
								1=1 AND 
								(signature LIKE '%[/url]%' OR signature LIKE '%[/ame]%' OR pagetext LIKE '%[/nomedia]%')
								$and 
							ORDER BY 
								userid DESC 
							LIMIT $limitstart, $perpage";
